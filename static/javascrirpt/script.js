...
(function ($) {
    ...
    $(".product-buttons a.btn-animation").on("click", function () {
        const notificationOptions = {
            icon: "fa fa-check",
            title: "Success!",
            message: "Item Successfully added to your cart",
    ...
    $(".table-cart-button a.btn-solid-blue").on("click", function () {
        const notificationOptions = {
            icon: "fa fa-check",
            title: "Success!",
            message: "Item Successfully added to your cart",
    ...
    $(document).ready(function () {
        ...
        if (storedNotification) {
            ...
            $.notify(notificationOptions, {
                ...
                icon_type: "class",
                template: '<div data-notify="container" class="col-xl-4 col-lg-5 col-md-7 col-sm-8 col-12 alert alert-{0}" role="alert">' +
                    '<button type="button" aria-hidden="true" class="btn-close" data-notify="dismiss"></button>' +
                    '<span data-notify="icon"></span> ' +
                    '<span data-notify="title">{1}</span> ' +
                    '<span data-notify="message">{2}</span>' +
                    '<div class="progress" data-notify="progressbar">' +
                    '<div class="progress-bar progress-bar-info progress-bar-{0}" role="progressbar" style="width: 0%;"></div>' +
                    "</div>" +
    ...
    // NEW JS ADDED For popup to show product added to compare list START ----------------------------------------------------------------
    /*=====================
       8.2. Add to compare list
       ==========================*/

    $(".product-box .cart-wrap li .add-to-compare-list").on("click", function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (isAuthenticated) {
                    // User is authenticated, show the success notification
                    $.notify({
                        icon: "fa fa-check",
                        title: "Success!",
                        message: "Item Successfully added to your compare list",
                    }, {
                        element: "body",
                        position: null,
                        type: "success",
    ...
    // NEW JS ADDED For popup to show product added to compare list END ----------------------------------------------------------------
    ...
    $(document).ready(function () {
        $.ajax({
            ...
            success: function (data) {
                ...
                if (isAuthenticated) {
                    $(".product-box a.wishlist, .product-buttons a.wishlist, .banner-detail a.wishlist, .add-btn a.wishlist, .product-box .top-wishlist a.wishlist").on("click", function () {
                        const notificationOptions = {
                            icon: "fa fa-check",
                            title: "Success!",
    ...
    // Check if there is a stored notification in local storage and display it
    $(document).ready(function () {
        const storedNotification = localStorage.getItem("wishlistpopup");
        if (storedNotification) {
            const notificationOptions = JSON.parse(storedNotification);
            $.notify(notificationOptions, {
                icon: "fa fa-check",
            ...
            localStorage.removeItem("wishlistpopup"); // Remove the notification after displaying
    ...
    /*=====================
      18. Coockie js
     ==========================*/
    $(".cookie-bar-section #cookiebutton").on("click", function () {
...
/*=====================
    26.  Set used color code in cookie js
 ==========================*/

var colorUsed = document.getElementById("colorUsed");

if (colorUsed !== null) {
  var computedStyle = window.getComputedStyle(colorUsed);
  var propertyValue = computedStyle.getPropertyValue('--theme-color');
  var existingValue = localStorage.getItem("colorValue");

  if (existingValue !== null) {
    localStorage.setItem("colorValue", propertyValue);
  } else {
    localStorage.setItem("colorValue", propertyValue);
  }
} else {
  console.warn("Element with ID 'colorUsed' not found on this page.");
}
